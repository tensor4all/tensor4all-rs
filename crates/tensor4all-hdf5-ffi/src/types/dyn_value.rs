...
use crate::types::h5type::{
    hvl_t, CompoundType, EnumType, FloatSize, H5Type, IntSize, TypeDescriptor,
};
use crate::types::string::{VarLenAscii, VarLenUnicode};
...
unsafe impl DynDrop for DynArray<'_> {
    fn dyn_drop(&mut self) {
        ...
        if self.len.is_none() && !self.get_ptr().is_null() {
            unsafe {
                crate::types::free(self.get_ptr() as *mut _);
            }
        }
    }
}
...
unsafe impl DynClone for DynArray<'_> {
    fn dyn_clone(&mut self, out: &mut [u8]) {
        ...
        let out = if self.len.is_none() {
            ...
            unsafe {
                let dst = crate::types::malloc(len * size).cast();
    ...
    fn dyn_clone(&mut self, out: &mut [u8]) {
        ...
        if !self.get_ptr().is_null() {
            unsafe {
                ...
                let dst = crate::types::malloc(raw_len + 1).cast();
