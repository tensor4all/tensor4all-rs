...
mod tests {
    ...
    #[test]
    fn test_vidal_mpo_accessors_and_rank() {
        let mpo = VidalMPO::from_parts_unchecked(
        ...
        assert_eq!(mpo.lambda(0), &[0.5, 0.25]);
        assert_eq!(mpo.lambdas(), &[vec![0.5, 0.25]]);
        assert_eq!(*mpo.gamma(0).get4(0, 0, 1, 0), 2.0);
        assert_eq!(mpo.gammas().len(), 2);
    }

    #[test]
    fn test_vidal_mpo_rank_for_single_site_is_one_and_mutation_works() {
        let mut mpo = VidalMPO::from_parts_unchecked(vec![sample_tensor4(5.0)], Vec::new());
        assert_eq!(mpo.rank(), 1);
        assert!(mpo.link_dims().is_empty());

        *mpo.gamma_mut(0).get4_mut(0, 0, 0, 0) = 9.0;
        assert_eq!(*mpo.gamma(0).get4(0, 0, 0, 0), 9.0);
    }
    ...
    #[test]
    fn test_vidal_mpo_into_mpo_is_currently_unimplemented() {
        let mpo = VidalMPO::from_parts_unchecked(vec![sample_tensor4(1.0)], Vec::new());
        assert!(matches!(
            mpo.into_mpo(),
            Err(MPOError::InvalidOperation { .. })
        ));
