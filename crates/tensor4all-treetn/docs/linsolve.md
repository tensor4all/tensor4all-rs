...
- **Linsolve implementation (a `LocalUpdater`)**:
  - `crates/tensor4all-treetn/src/treetn/linsolve/updater.rs`
  - Type: `LinsolveUpdater<T, V>`
...
2. For each `LocalUpdateStep { nodes, new_center }`:
   - `LinsolveUpdater::before_step(step, full_treetn_before)` (lazy init / pre-step hooks)
   - extract a local subtree (region)
   - call `LinsolveUpdater::update(subtree, step, full_treetn)`
   - insert the updated subtree back
   - move canonical center to `new_center`
   - call `LinsolveUpdater::after_step(step, full_treetn_after)` (bra sync + cache invalidation)

## What happens inside `LinsolveUpdater::update`

Source: `crates/tensor4all-treetn/src/treetn/linsolve/updater.rs`
...
- `LinsolveUpdater::solve_local(region, init_local, full_treetn)`
...
Relevant files:
- `crates/tensor4all-treetn/src/treetn/linsolve/updater.rs`
- `crates/tensor4all-treetn/src/treetn/linsolve/local_linop.rs`
...
Relevant files:
- `crates/tensor4all-treetn/src/treetn/linsolve/local_linop.rs`
- `crates/tensor4all-treetn/src/treetn/linsolve/projected_operator.rs`
- `crates/tensor4all-treetn/src/treetn/linsolve/environment.rs` (cache structure)

## Bra/Ket separation (current implementation)

To avoid unintended bra↔ket contractions (Issue #173 context) while still reusing cached environments,
linsolve keeps a persistent bra network:

- `LinsolveUpdater` owns a `bra_state: TreeTN<T, V>` whose **link indices use different IDs** from
  the ket (`full_treetn`).
- On each step, `after_step` updates the bra region to track the updated ket region while keeping
...
Operationally:
- `solve_local` constructs `LocalLinOp::with_bra_state(...)`, so
  `ProjectedOperator::apply(..., ket_state=state, bra_state=self.bra_state, ...)` uses distinct
  bra/ket link namespaces and avoids spurious traces when contracting with `AllowedPairs::All`.
...
Relevant file:
- `crates/tensor4all-treetn/src/treetn/linsolve/projected_state.rs`
...
- `crates/tensor4all-treetn/src/treetn/linsolve/updater.rs` (copy back logic)
...
- `LinsolveUpdater::after_step(step, full_treetn_after)`

This performs (1) bra synchronization for the updated region and then (2) invalidates environment
caches in both:
...
## Known pitfall: unintended contraction due to shared index IDs (Issue #173 context)
...
apply_local_update_sweep(state: TreeTN, plan, updater)
  └─ for step in plan.steps:
       ├─ LinsolveUpdater::before_step(step, full_treetn_before)
       │    └─ ensure_bra_state_initialized(full_treetn_before)   // lazy init
       ├─ (extract subtree for step.nodes)            // localupdate infrastructure
       ├─ LinsolveUpdater::update(subtree, step, full_treetn)
       ...
       │    │    ├─ linop = LocalLinOp::with_bra_state(projected_operator, region, state.clone(), bra_state.clone(), a0, a1)
       │    │    └─ gmres(apply_a, rhs_local, init_local, gmres_options)
       │    │         └─ apply_a(x_local: &T) = LocalLinOp::apply(x_local)
       │    │              ├─ ProjectedOperator::apply(v=x_local, region, ket_state=state, bra_state=bra_state, topology)
       │    │              │    ├─ ensure_environments(...)
       │    │              │    │    └─ compute_environment(from, to, ket_state, bra_state, topology)   // recursive
       ...
       └─ LinsolveUpdater::after_step(step, full_treetn_after)
            ├─ sync_bra_state_region(step, full_treetn_after)     // keep boundary bonds stable
