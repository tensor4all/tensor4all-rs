    ...
    V: Clone + Hash + Eq + Send + Sync + std::fmt::Debug,
{
    ...
    /// Replace all link/bond indices with fresh IDs using a provided RNG.
    ///
    /// This is the deterministic version of [`sim_linkinds`](Self::sim_linkinds) that uses
    /// the provided random number generator instead of the thread-local RNG.
    /// Use this when you need reproducible results with the same seed.
    ///
    /// # Arguments
    /// * `rng` - A random number generator to use for ID generation
    pub fn sim_linkinds_with_rng<R: rand::Rng>(&self, rng: &mut R) -> Result<Self>
    where
        T::Index: IndexLike,
    {
        let mut result = self.clone();
        result.sim_linkinds_mut_with_rng(rng)?;
        Ok(result)
    }

    /// Replace all link/bond indices with fresh IDs in-place using a provided RNG.
    ///
    /// This is the deterministic version of [`sim_linkinds_mut`](Self::sim_linkinds_mut).
    ///
    /// # Arguments
    /// * `rng` - A random number generator to use for ID generation
    pub fn sim_linkinds_mut_with_rng<R: rand::Rng>(&mut self, rng: &mut R) -> Result<()>
    where
        T::Index: IndexLike,
    {
        // Snapshot edges first since replacements may touch internal maps.
        ...
        for edge in edges {
            let old_bond = self
                ...
                .ok_or_else(|| anyhow::anyhow!("Bond index not found for edge {:?}", edge))?
                .clone();
            let new_bond = old_bond.sim_with_rng(rng);
