...
fn test_comprehensive_functionality() {
    ...
    // With includeendpoint, upper_bound is adjusted internally
    let ub = grid.upper_bound();
    for d in 0..4 {
        assert!(ub[d] > upper_bound[d] || (ub[d] - upper_bound[d]).abs() < 1e-10);
    }

    // grid_max should approximate upper_bound
    for d in 0..4 {
        assert!((gmax[d] - upper_bound[d]).abs() < 1e-6);
    }
