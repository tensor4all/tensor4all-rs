...
use tensor4all_core_linalg::{qr, qr_c64};
...
fn test_qr_identity() {
    ...
    let storage = Arc::new(Storage::DenseF64(
        tensor4all_core_tensor::storage::DenseStorageF64::from_vec(data),
    ));
...
fn test_qr_reconstruction() {
    ...
    let storage = Arc::new(Storage::DenseF64(
        tensor4all_core_tensor::storage::DenseStorageF64::from_vec(data.clone()),
    ));
    let tensor: TensorDynLen<DynId> =
    ...
    let reconstructed_storage = Arc::new(Storage::DenseF64(
        tensor4all_core_tensor::storage::DenseStorageF64::from_vec(reconstructed_data),
    ));
...
fn test_qr_complex_reconstruction() {
    ...
    let storage = Arc::new(Storage::DenseC64(
        tensor4all_core_tensor::storage::DenseStorageC64::from_vec(data.clone()),
    ));
    let tensor: TensorDynLen<DynId> =
