//! Benchmark for stable-pre-tenferro branch.
//! Run: cargo test --release -p tensor4all-itensorlike --test bench_basic_ops_old -- --nocapture
...
fn bench_norm() {
    ...
    for &n in &[20, 45, 90] {
        ...
        time_it(&format!("norm({n} sites, bd=16)"), || { let _ = mps.norm(); });
    }
}
...
fn bench_inner() {
    ...
    for &n in &[20, 45, 90] {
        ...
        time_it(&format!("inner({n} sites, bd=16)"), || { let _ = a.inner(&b); });
    }
}

#[test]
fn bench_truncate() {
    eprintln!("\n=== truncate benchmark (old) ===");
    for &n in &[20, 45, 90] {
        let mut mps = make_random_mps(n, 2, 16, 123);
        let opts = TruncateOptions::svd().with_rtol(1e-6).with_max_rank(8);
        time_it(&format!("truncate({n} sites, bd=16→8)"), || { mps.truncate(&opts).unwrap(); });
    }
}

#[test]
fn bench_contract_zipup() {
    eprintln!("\n=== contract zipup benchmark (old) ===");
    for &n in &[10, 20, 45] {
        let (a, b) = make_random_mpo_pair(n, 2, 8);
        let opts = ContractOptions::zipup().with_rtol(1e-6).with_max_rank(16);
        time_it(&format!("zipup({n} sites, bd=8)"), || { let _ = a.contract(&b, &opts).unwrap(); });
    }
}

#[test]
fn bench_contract_fit() {
    eprintln!("\n=== contract fit benchmark (old) ===");
    for &n in &[10, 20, 45] {
        let (a, b) = make_random_mpo_pair(n, 2, 8);
        let opts = ContractOptions::fit().with_rtol(1e-6).with_max_rank(16);
        time_it(&format!("fit({n} sites, bd=8)"), || { let _ = a.contract(&b, &opts).unwrap(); });
    }
}
