...
/// - For x_n = 0: 1
/// - For x_n = 1: exp(i*θ*2^n)
///
/// Uses little-endian convention: site n corresponds to bit 2^n (LSB at site 0).
/// This matches the convention used by shift and flip operators.
fn phase_rotation_mpo(r: usize, theta: f64) -> Result<TensorTrain<Complex64>> {
    ...
    for n in 0..r {
        // Phase for this site: θ * 2^n (little-endian: site 0 = LSB)
...
mod tests {
    ...
    fn test_phase_rotation_pi() {
        // θ = π should give (-1)^x
        // With little-endian: site 0 has phase π * 2^0 = π, so exp(i*π) = -1
        let r = 3;
        let mpo = phase_rotation_mpo(r, PI).unwrap();

        // For site 0 (LSB), phase = π * 2^0 = π, exp(i*π) = -1
        ...
        // For site 2 (MSB), phase = π * 2^2 = 4π ≡ 0 (mod 2π), exp(0) = 1
